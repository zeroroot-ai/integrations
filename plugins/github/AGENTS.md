...
This directory is a **Gibson plugin**: a Go service that integrates GitHub into
...
core function `f(ctx, *github.Client, Req) (Resp, error)`, plus a thin handler
...
The GitHub token is resolved from the SDK secrets broker
(`plugin/secret.go`, `plugin/secrets/`) via `plugin.ResolveSecret(ctx,
"cred:github_token")`. Declare it in `plugin.yaml` under `spec.secrets`. Never
...
HTTP calls through a cassette-backed `*github.Client`. Tests run with a plain
